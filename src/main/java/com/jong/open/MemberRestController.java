...
public class MemberRestController {
	...
	@RequestMapping(method = { RequestMethod.PUT,
			RequestMethod.PATCH }, value = "/modify/{idx}", consumes = "application/json", produces = {
					MediaType.TEXT_PLAIN_VALUE })
	public ResponseEntity<String> modify(@RequestBody Member member, @PathVariable("idx") int idx) {
		log.info(member);
		member.setIdx(idx);

		return service.modify(member) ? new ResponseEntity<>("success", HttpStatus.OK)
				: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<String> remove(@PathVariable("idx") int idx) {
		return service.delete(idx) ? new ResponseEntity<>("success", HttpStatus.OK)
				: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
